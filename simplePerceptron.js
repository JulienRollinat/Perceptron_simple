 ...
 * 'tf.sequential()' est la forme la plus simple des modèles
 * La forme générique est 'tf.model()'
 */
...
/*
 * Fonction pour sauvegarder localement le modèle dans le 'localStorge' du navigateur
 */
const saveResults = async () => await model.save('localstorage://model_1');

/*
 * Fonction pour affiche la vleur des poids dans la console
 */
...
async function classify(model, sample) {
  ...
  tf.tidy(() => {
    ...
    const predictOut = model.predict(input, {verbose: true});
